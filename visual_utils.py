...
def plot_total_temps(filename: str):
    ...
    ax.annotate(f'{x[min_indices[0]]}', xy=(x[min_indices[0]], y[min_indices[0]]), xytext=(x[min_indices[0]]+0.02, y[min_indices[0]]+0.01),
            arrowprops=dict(facecolor='black', shrink=0.05))
    ax.axvline(x[min_indices[1]], linewidth=3, c='r', linestyle="dashed")
    ax.annotate(f'{x[min_indices[1]]}', xy=(x[min_indices[1]], y[min_indices[1]]), xytext=(x[min_indices[1]]+0.02, y[min_indices[1]]+0.01),
            arrowprops=dict(facecolor='black', shrink=0.05))
    ax.set_xlim(*XLIM)
    ax.set_ylim(*YLIM)
